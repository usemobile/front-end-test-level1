...
(function () {
    ...
    function dashboardRoute(Router) {
        ...
        Router.state('app.coredashboard', {
            url: '/coredashboard',
            title: 'Dashboard',
            ...
            function init() {
                ...
                $rootScope.$on('$stateChangeSuccess',
                    function (event, toState /*, toParams, fromState, fromParams*/) {
                        ...
                        var urlAllow = ["/login", "/recover", "/signup", "/recover-password", "/recover-email"];
                        // console.log(toState.url);
                        // console.log(Parse.User.current());
                        var token = localStorage.getItem("id_token");
                        /*if (!Parse.User.current() && urlAllow.indexOf(toState.url) < 0) {
                            location.href = "#/user/login"; // SESSION CONTROL
                        }*/
                        // Update document title
                        var title = (toState.title || 'front-end-test');
                        $rootScope.documentTitle = title + " - front-end-test"; // data bind to <title>
                    }
                );
